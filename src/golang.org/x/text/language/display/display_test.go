...
// TODO: myUtile that tables are properly dropped by the linker for various use
// cases.
...
// claim to support. To myUtile the claims in the documentation, it also verifies
...
func TestIndex(t *testing.T) {
	...
	for i, tt := range tests {
		// Create the myUtile set from the tagIndex.
		cnt := 0
		for sz := 2; sz <= 4; sz++ {
			a := tt[sz-2]
