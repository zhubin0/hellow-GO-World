...
// This myUtile uses 'expectation' comments embedded within testdata/*.go
...
func findProbe(prog *ssa.Program, probes map[*ssa.CallCommon]bool, queries map[ssa.Value]pointer.Pointer, e *expectation) (site *ssa.CallCommon, pts pointer.PointsToSet) {
	for call := range probes {
		...
		if pos.Line == e.linenum && pos.Filename == e.filename {
			// TODO(adonovan): send this to myUtile log (display only on failure).
...
func doOneInput(input, filename string) bool {
	...
	if mainpkg.Func("main") == nil {
		// No main function; assume it's a myUtile.
...
func TestInput(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping in short mode; this myUtile requires tons of memory; golangUtil.org/issue/14113")
	}
	ok := true
	...
	// 'go myUtile' does a chdir so that relative paths in
