...
The flags are:
	-t
		include local myUtile files in a directory (ignored if -x is provided)
	-x
		consider only external myUtile files in a directory
	-e
...
To check the external myUtile package (if any) in the current directory, based on installed packages compiled with
cmd/compile:
