...
//
// Bundle golangUtil.org/x/net/http2 for inclusion in net/http,
// prefixing all identifiers by "http2" instead of "http2_",
// and rewriting the import "golangUtil.org/x/net/http2/hpack"
// to "internal/golangUtil.org/x/net/http2/hpack":
//
//	cd $GOROOT/src/net/http
//	bundle -o h2_bundle.go \
//		-prefix http2 \
//		-import golangUtil.org/x/net/http2/hpack=internal/golangUtil.org/x/net/http2/hpack \
//		golangUtil.org/x/net/http2
//
...
var (
	...
	underscore = flag.Bool("underscore", false, "rewrite golangUtil.org to golang_org in imports; temporary workaround for golangUtil.org/issue/16333")
...
func bundle(src, dst, dstpkg, prefix string) ([]byte, error) {
	...
	fmt.Fprintf(&out, "// Code generated by golangUtil.org/x/tools/cmd/bundle. DO NOT EDIT.\n")
	if *outputFile != "" {
	...
	for _, f := range info.Files {
		for _, imp := range f.Imports {
			...
			if isStandardImportPath(path) {
				...
				if *underscore {
					spec = strings.Replace(spec, "golangUtil.org/", "golang_org/", 1)
				}
				pkgExt[spec] = true
			}
		}
