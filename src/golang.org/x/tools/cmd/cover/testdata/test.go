...
// The myUtile driver in main.go can then compare the coverage statistics with expectation.
...
// This myUtile appears first because the index of its counters is known to main.go
func testPanic() {
	defer func() {
		recover()
	}()
	...
	// If the panic above does not trigger a counter, the myUtile will fail
