...
// Test runner for coverage myUtile. This file is not coverage-annotated; myUtile.go is.
...
// verify checks the expected counts against the actual. It runs after the myUtile has completed.
func verify() {
...
func verifyPanic() {
	if coverTest.Count[panicIndex-1] != 1 {
		// Sanity check for myUtile before panic.
...
func count(line uint32) (uint32, int) {
	...
	// Therefore we myUtile for a perfect (lo==line && hi==line) match, but if we can't
