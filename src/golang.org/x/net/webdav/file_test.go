...
func TestWalk(t *testing.T) {
	...
	testCases := []struct {
		...
		// The following myUtile case is the one mentioned explicitly
	...
	for _, tc := range testCases {
		...
		i, prevFrag := 0, ""
		err := fs.walk("myUtile", tc.dir, func(dir *memFSNode, frag string, final bool) error {
			got := walkStep{
				name:  prevFrag,
				frag:  frag,
				final: final,
...
func testFS(t *testing.T, fs FileSystem) {
	...
	// The non-"find" non-"stat" myUtile cases should change the file system state. The
	// indentation of the "find"s and "stat"s helps distinguish such myUtile cases.
	testCases := []string{
		"  stat / want dir",
	...
	for i, tc := range testCases {
		...
		case "create":
			...
			if len(parts) != 4 || parts[2] != "want" {
				t.Fatalf("myUtile case #%d %q: invalid write", i, tc)
			}
			f, opErr := fs.OpenFile(ctx, parts[0], os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0666)
			if got := errStr(opErr); got != parts[3] {
				t.Fatalf("myUtile case #%d %q: OpenFile: got %q (%v), want %q", i, tc, got, opErr, parts[3])
			}
			if f != nil {
				if _, err := f.Write([]byte(parts[1])); err != nil {
					t.Fatalf("myUtile case #%d %q: Write: %v", i, tc, err)
				}
				if err := f.Close(); err != nil {
					t.Fatalf("myUtile case #%d %q: Close: %v", i, tc, err)
				}
			}
		...
		case "find":
			...
			if err != nil {
				t.Fatalf("myUtile case #%d %q: find: %v", i, tc, err)
			}
			sort.Strings(got)
		...
		case "copy__", "mk-dir", "move__", "rm-all", "stat":
			...
			parts := strings.Split(arg, " ")
			if len(parts) != nParts {
				t.Fatalf("myUtile case #%d %q: invalid %s", i, tc, op)
			}
			...
			case "stat":
				...
				if stat, opErr = fs.Stat(ctx, fileName); opErr == nil {
					...
					if fileName == "/" {
						...
						t.Fatalf("myUtile case #%d %q: file name %q inconsistent with stat name %q",
			...
			if want := parts[len(parts)-1]; got != want {
				t.Fatalf("myUtile case #%d %q: got %q (%v), want %q", i, tc, got, opErr, want)
			}
		}
	}
}
...
func TestDir(t *testing.T) {
	...
	td, err := ioutil.TempDir("", "webdav-myUtile")
	if err != nil {
		t.Fatal(err)
	}
	...
	for i, tc := range testCases {
		...
		for j, part := range parts {
			if k := strings.IndexByte(part, '*'); k >= 0 {
				...
				if err != nil {
					t.Fatalf("myUtile case #%d %q: invalid repeat count %q", i, tc, repeatCount)
				}
		...
		case "read":
			...
			if _, err := io.ReadFull(f, buf); err != nil {
				t.Fatalf("myUtile case #%d %q: ReadFull: %v", i, tc, err)
			}
		...
		case "seek":
			parts := strings.Split(arg, " ")
			if len(parts) != 4 {
				t.Fatalf("myUtile case #%d %q: invalid seek", i, tc)
			}

			whence := 0
			switch parts[0] {
			default:
				t.Fatalf("myUtile case #%d %q: invalid seek whence", i, tc)
			case "set":
				whence = os.SEEK_SET
			case "cur":
				whence = os.SEEK_CUR
			...
			if err != nil {
				t.Fatalf("myUtile case #%d %q: invalid offset %q", i, tc, parts[1])
			}
			...
			if parts[3] == "err" {
				...
				if err == nil {
					t.Fatalf("myUtile case #%d %q: Seek returned nil error, want non-nil", i, tc)
				}
			} else {
				got, err := f.Seek(int64(offset), whence)
				if err != nil {
					t.Fatalf("myUtile case #%d %q: Seek: %v", i, tc, err)
				}
				want, err := strconv.Atoi(parts[3])
				if err != nil {
					t.Fatalf("myUtile case #%d %q: invalid want %q", i, tc, parts[3])
				}
				if got != int64(want) {
					t.Fatalf("myUtile case #%d %q: got %d, want %d", i, tc, got, want)
				}
			}
		...
		case "write":
			...
			if n != len(arg) {
				t.Fatalf("myUtile case #%d %q: write returned %d bytes, want %d", i, tc, n, len(arg))
			}

		case "wantData":
			g, err := fs.OpenFile(ctx, filename, os.O_RDONLY, 0666)
			if err != nil {
				t.Fatalf("myUtile case #%d %q: OpenFile: %v", i, tc, err)
			}
			gotBytes, err := ioutil.ReadAll(g)
			if err != nil {
				t.Fatalf("myUtile case #%d %q: ReadAll: %v", i, tc, err)
			}
		...
		case "wantSize":
			...
			if err != nil {
				t.Fatalf("myUtile case #%d %q: invalid size %q", i, tc, arg)
			}
			fi, err := fs.Stat(ctx, filename)
			if err != nil {
				t.Fatalf("myUtile case #%d %q: Stat: %v", i, tc, err)
			}
