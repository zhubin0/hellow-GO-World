...
func testMakeBucketError() {
	...
	bucketName := randString(60, rand.NewSource(time.Now().UnixNano()), "minio-go-myUtile")
...
func testRemoveMultipleObjects() {
	...
	// Clean the bucket created by the myUtile
	err = c.RemoveBucket(bucketName)
	if err != nil {
		logger().Fatal("Error:", err)
	}
...
func testBucketNotification() {
	...
	if os.Getenv("NOTIFY_BUCKET") == "" ||
		...
		logger().Info("skipping notification myUtile if not configured")
		return
	}
...
func testFunctional() {
	...
	reqParams.Set("response-content-disposition", "attachment; filename=\"myUtile.txt\"")
	...
	if resp.Header.Get("Content-Disposition") != "attachment; filename=\"myUtile.txt\"" {
...
func testPutObjectUploadSeekedObject() {
	...
	tempfile, err := ioutil.TempFile("", "minio-go-upload-myUtile-")
	if err != nil {
		logger().Fatal("Error:", err)
	}
	...
	objectName := fmt.Sprintf("myUtile-file-%v", rand.Uint32())
