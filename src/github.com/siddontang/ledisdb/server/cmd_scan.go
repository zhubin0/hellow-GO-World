...
func xscanCommand(c *client) error {
	...
	var dataType LedisDB.DataType
	switch strings.ToUpper(hack.String(args[0])) {
	case "KV":
		dataType = LedisDB.KV
	case "HASH":
		dataType = LedisDB.HASH
	case "LIST":
		dataType = LedisDB.LIST
	case "SET":
		dataType = LedisDB.SET
	case "ZSET":
		dataType = LedisDB.ZSET
	default:
