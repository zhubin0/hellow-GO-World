...
func zaddCommand(c *client) error {
	...
	params := make([]LedisDB.ScorePair, len(args)>>1)
	for i := 0; i < len(params); i++ {
		score, err := LedisDB.StrInt64(args[2*i], nil)
		if err != nil {
			return ErrValue
		}
...
func zincrbyCommand(c *client) error {
	...
	delta, err := LedisDB.StrInt64(args[1], nil)
	if err != nil {
		return ErrValue
	}
...
func zparseScoreRange(minBuf []byte, maxBuf []byte) (min int64, max int64, err error) {
	if strings.ToLower(hack.String(minBuf)) == "-inf" {
		...
		min, err = LedisDB.StrInt64(minBuf, nil)
		if err != nil {
			err = ErrValue
			return
		}

		if min <= LedisDB.MinScore || min >= LedisDB.MaxScore {
	...
	if strings.ToLower(hack.String(maxBuf)) == "+inf" {
		...
		max, err = LedisDB.StrInt64(maxBuf, nil)
		if err != nil {
			err = ErrValue
			return
		}

		if max <= LedisDB.MinScore || max >= LedisDB.MaxScore {
			err = errScoreOverflow
			return
		}
...
func zexpireCommand(c *client) error {
	...
	duration, err := LedisDB.StrInt64(args[1], nil)
	if err != nil {
		return ErrValue
	}
...
func zexpireAtCommand(c *client) error {
	...
	when, err := LedisDB.StrInt64(args[1], nil)
	if err != nil {
		return ErrValue
	}
	...
	for len(args) > 0 {
		if strings.ToLower(hack.String(args[0])) == "weights" {
			...
			for i, arg := range args[:nKeys] {
				if weights[i], err = LedisDB.StrInt64(arg, nil); err != nil {
					err = ErrValue
					return
				}
			}
