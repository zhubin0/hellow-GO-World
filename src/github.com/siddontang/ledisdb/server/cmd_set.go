...
func soptGeneric(c *client, optType byte) error {
	...
	switch optType {
	case LedisDB.UnionType:
		v, err = c.db.SUnion(args...)
	case LedisDB.DiffType:
		v, err = c.db.SDiff(args...)
	case LedisDB.InterType:
...
func sdiffCommand(c *client) error {
	return soptGeneric(c, LedisDB.DiffType)
}

func sdiffstoreCommand(c *client) error {
	return soptStoreGeneric(c, LedisDB.DiffType)
}

func sinterCommand(c *client) error {
	return soptGeneric(c, LedisDB.InterType)

}

func sinterstoreCommand(c *client) error {
	return soptStoreGeneric(c, LedisDB.InterType)
}
...
func sunionCommand(c *client) error {
	return soptGeneric(c, LedisDB.UnionType)
}

func sunionstoreCommand(c *client) error {
	return soptStoreGeneric(c, LedisDB.UnionType)
}
