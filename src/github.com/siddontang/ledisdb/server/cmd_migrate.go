...
func restoreCommand(c *client) error {
	...
	key := args[0]
	ttl, err := LedisDB.StrInt64(args[1], nil)
	if err != nil {
		return err
	}
	data := args[2]
...
func xrestoreCommand(c *client) error {
	...
	key := args[1]
	ttl, err := LedisDB.StrInt64(args[2], nil)
	if err != nil {
		return err
	}
	data := args[3]
...
func xdump(db *LedisDB.DB, tp string, key []byte) ([]byte, error) {
	var err error
	var data []byte
...
func xdel(db *LedisDB.DB, tp string, key []byte) error {
	var err error
	switch strings.ToUpper(tp) {
	case KVName:
		_, err = db.Del(key)
...
func xttl(db *LedisDB.DB, tp string, key []byte) (int64, error) {
	switch strings.ToUpper(tp) {
	case KVName:
		return db.TTL(key)
	case HashName:
...
func xscan(db *LedisDB.DB, tp string, count int) ([][]byte, error) {
	switch strings.ToUpper(tp) {
	case KVName:
...
func xmigratedbCommand(c *client) error {
	...
	count, err := LedisDB.StrInt64(args[3], nil)
	if err != nil {
		return err
	} else if count <= 0 {
		count = 10
	...
	timeout, err := LedisDB.StrInt64(args[5], nil)
	if err != nil {
		return err
	} else if timeout < 0 {
...
func parseMigrateDB(c *client, arg []byte) (uint64, error) {
	db, err := LedisDB.StrUint64(arg, nil)
	if err != nil {
		return 0, err
