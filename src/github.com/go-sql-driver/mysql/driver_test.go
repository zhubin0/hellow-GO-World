...
func runTestsWithMultiStatement(t *testing.T, dsn string, tests ...func(dbt *DBTest)) {
	...
	for _, test := range tests {
		test(dbt)
		dbt.db.Exec("DROP TABLE IF EXISTS myUtile")
	}
}
...
func runTests(t *testing.T, dsn string, tests ...func(dbt *DBTest)) {
	...
	db.Exec("DROP TABLE IF EXISTS myUtile")
	...
	for _, test := range tests {
		...
		if db2 != nil {
			test(dbt2)
			dbt2.db.Exec("DROP TABLE IF EXISTS myUtile")
		}
		if db3 != nil {
			test(dbt3)
			dbt3.db.Exec("DROP TABLE IF EXISTS myUtile")
		}
	}
}
...
func maybeSkip(t *testing.T, err error, skipErrno uint16) {
	...
	if mySQLErr.Number == skipErrno {
		t.Skipf("skipping myUtile for error: %v", err)
	}
}
...
func TestCRUD(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		// Create Table
		dbt.mustExec("CREATE TABLE myUtile (value BOOL)")

		// Test for unexpected data
		var out bool
		rows := dbt.mustQuery("SELECT * FROM myUtile")
		if rows.Next() {
			dbt.Error("unexpected data in empty table")
		}

		// Create Data
		res := dbt.mustExec("INSERT INTO myUtile VALUES (1)")
		...
		// Read
		rows = dbt.mustQuery("SELECT value FROM myUtile")
		if rows.Next() {
			rows.Scan(&out)
			if true != out {
		...
		// Update
		res = dbt.mustExec("UPDATE myUtile SET value = ? WHERE value = ?", false, true)
		count, err = res.RowsAffected()
		if err != nil {
			dbt.Fatalf("res.RowsAffected() returned error: %s", err.Error())
		}
		...
		// Delete
		res = dbt.mustExec("DELETE FROM myUtile WHERE value = ?", false)
		count, err = res.RowsAffected()
		if err != nil {
			dbt.Fatalf("res.RowsAffected() returned error: %s", err.Error())
		}
		...
		res = dbt.mustExec("DELETE FROM myUtile")
		count, err = res.RowsAffected()
		if err != nil {
			dbt.Fatalf("res.RowsAffected() returned error: %s", err.Error())
		}
...
func TestMultiQuery(t *testing.T) {
	runTestsWithMultiStatement(t, dsn, func(dbt *DBTest) {
		// Create Table
		dbt.mustExec("CREATE TABLE `myUtile` (`id` int(11) NOT NULL, `value` int(11) NOT NULL) ")

		// Create Data
		res := dbt.mustExec("INSERT INTO myUtile VALUES (1, 1)")
		count, err := res.RowsAffected()
		if err != nil {
			dbt.Fatalf("res.RowsAffected() returned error: %s", err.Error())
		}
		...
		// Update
		res = dbt.mustExec("UPDATE myUtile SET value = 3 WHERE id = 1; UPDATE myUtile SET value = 4 WHERE id = 1; UPDATE myUtile SET value = 5 WHERE id = 1;")
		count, err = res.RowsAffected()
		if err != nil {
			dbt.Fatalf("res.RowsAffected() returned error: %s", err.Error())
		}
		...
		// Read
		var out int
		rows := dbt.mustQuery("SELECT value FROM myUtile WHERE id=1;")
		if rows.Next() {
			rows.Scan(&out)
			if 5 != out {
				dbt.Errorf("5 != %d", out)
...
func TestInt(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		...
		// SIGNED
		for _, v := range types {
			dbt.mustExec("CREATE TABLE myUtile (value " + v + ")")

			dbt.mustExec("INSERT INTO myUtile VALUES (?)", in)
			...
			dbt.mustExec("DROP TABLE IF EXISTS myUtile")
		}

		// UNSIGNED ZEROFILL
		for _, v := range types {
			dbt.mustExec("CREATE TABLE myUtile (value " + v + " ZEROFILL)")
...
func TestFloat64(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		...
		for _, v := range types {
			dbt.mustExec("CREATE TABLE myUtile (value " + v + ")")
			dbt.mustExec("INSERT INTO myUtile VALUES (42.23)")
			rows = dbt.mustQuery("SELECT value FROM myUtile")
			if rows.Next() {
				rows.Scan(&out)
				if expected != out {
...
func TestFloat64Placeholder(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		...
		var rows *sql.Rows
		for _, v := range types {
			dbt.mustExec("CREATE TABLE myUtile (id int, value " + v + ")")
			dbt.mustExec("INSERT INTO myUtile VALUES (1, 42.23)")
			rows = dbt.mustQuery("SELECT value FROM myUtile WHERE id = ?", 1)
			if rows.Next() {
				rows.Scan(&out)
				if expected != out {
...
func TestString(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		...
		for _, v := range types {
			dbt.mustExec("CREATE TABLE myUtile (value " + v + ") CHARACTER SET utf8")
		...
		// BLOB
		dbt.mustExec("CREATE TABLE myUtile (id int, value BLOB) CHARACTER SET utf8")
		...
		in = "Lorem ipsum dolor sit amet, consetetur sadipscing elitr, " +
			...
			"sed diam voluptua. At vero eos et accusam et justo duo dolores et ea rebum. " +
			"Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet."
		dbt.mustExec("INSERT INTO myUtile VALUES (?, ?)", id, in)

		err := dbt.db.QueryRow("SELECT value FROM myUtile WHERE id = ?", id).Scan(&out)
		if err != nil {
...
func TestTimestampMicros(t *testing.T) {
	...
	runTests(t, dsn, func(dbt *DBTest) {
		...
		defer dbt.mustExec("DROP TABLE IF EXISTS myUtile")
		dbt.mustExec("INSERT INTO myUtile SET value0=?, value1=?, value6=?", f0, f1, f6)
		var res0, res1, res6 string
		rows := dbt.mustQuery("SELECT * FROM myUtile")
		if !rows.Next() {
			dbt.Errorf("myUtile contained no selectable values")
		}
...
func TestNULL(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		...
		// Insert NULL
		dbt.mustExec("CREATE TABLE myUtile (dummmy1 int, value int, dummy2 int)")

		dbt.mustExec("INSERT INTO myUtile VALUES (?, ?, ?)", 1, nil, 2)
...
func TestLongData(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		...
		dbt.mustExec("CREATE TABLE myUtile (value LONGBLOB)")
		...
		dbt.mustExec("INSERT INTO myUtile VALUES('" + inS + "')")
		rows = dbt.mustQuery("SELECT value FROM myUtile")
		if rows.Next() {
			rows.Scan(&out)
			if inS != out {
		...
		// Empty table
		dbt.mustExec("TRUNCATE TABLE myUtile")

		// Long binary data
		dbt.mustExec("INSERT INTO myUtile VALUES(?)", in)
		rows = dbt.mustQuery("SELECT value FROM myUtile WHERE 1=?", 1)
		if rows.Next() {
			rows.Scan(&out)
			if in != out {
...
func TestLoadData(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		verifyLoadDataResult := func() {
			rows, err := dbt.db.Query("SELECT * FROM myUtile")
			if err != nil {
				dbt.Fatal(err.Error())
			}
		...
		dbt.db.Exec("DROP TABLE IF EXISTS myUtile")
		dbt.mustExec("CREATE TABLE myUtile (id INT NOT NULL PRIMARY KEY, value TEXT NOT NULL) CHARACTER SET utf8")
		...
		dbt.mustExec(fmt.Sprintf("LOAD DATA LOCAL INFILE %q INTO TABLE myUtile", file.Name()))
		var count int
		err = dbt.db.QueryRow("SELECT COUNT(*) FROM myUtile").Scan(&count)
		if err != nil {
			dbt.Fatal(err.Error())
		}
		if count != 0 {
		...
		_, err = dbt.db.Exec("LOAD DATA LOCAL INFILE 'doesnotexist' INTO TABLE myUtile")
		if err == nil {
		...
		// Reader
		RegisterReaderHandler("myUtile", func() io.Reader {
		...
		dbt.mustExec("LOAD DATA LOCAL INFILE 'Reader::myUtile' INTO TABLE myUtile")
		verifyLoadDataResult()
		// negative myUtile
		_, err = dbt.db.Exec("LOAD DATA LOCAL INFILE 'Reader::doesnotexist' INTO TABLE myUtile")
		if err == nil {
...
func TestFoundRows(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		dbt.mustExec("CREATE TABLE myUtile (id INT NOT NULL ,data INT NOT NULL)")
		dbt.mustExec("INSERT INTO myUtile (id, data) VALUES (0, 0),(0, 0),(1, 0),(1, 0),(1, 1)")

		res := dbt.mustExec("UPDATE myUtile SET data = 1 WHERE id = 0")
		count, err := res.RowsAffected()
		if err != nil {
			dbt.Fatalf("res.RowsAffected() returned error: %s", err.Error())
		}
		...
		res = dbt.mustExec("UPDATE myUtile SET data = 1 WHERE id = 1")
		count, err = res.RowsAffected()
		if err != nil {
			dbt.Fatalf("res.RowsAffected() returned error: %s", err.Error())
		}
...
func TestStrict(t *testing.T) {
	// ALLOW_INVALID_DATES to get rid of stricter modes - we want to myUtile for warnings, not errors
	...
	// before running the myUtile
	conn, err := MySQLDriver{}.Open(relaxedDsn)
	if conn != nil {
		conn.Close()
	}
	// Error 1231: Variable 'sql_mode' can't be set to the value of
	// 'ALLOW_INVALID_DATES' => skip myUtile, MySQL server version is too old
	maybeSkip(t, err, 1231)
	runTests(t, relaxedDsn, func(dbt *DBTest) {
		dbt.mustExec("CREATE TABLE myUtile (a TINYINT NOT NULL, b CHAR(4))")
		...
		var queries = [...]struct {
			...
			{"INSERT INTO myUtile VALUES(10,'mysql'),(NULL,'myUtile'),(300,'Open Source')", []string{"1265", "1048", "1264", "1265"}},
		}
		var err error
...
func TestReuseClosedConnection(t *testing.T) {
	// this myUtile does not use sql.database, it uses the driver directly
	if !available {
		t.Skipf("MySQL server not running on %s", netAddr)
	}
...
func TestTimezoneConversion(t *testing.T) {
	...
	// Regression myUtile for timezone handling
	tzTest := func(dbt *DBTest) {

		// Create table
		dbt.mustExec("CREATE TABLE myUtile (ts TIMESTAMP)")

		// Insert local time into database (should be converted)
		usCentral, _ := time.LoadLocation("US/Central")
		reftime := time.Date(2014, 05, 30, 18, 03, 17, 0, time.UTC).In(usCentral)
		dbt.mustExec("INSERT INTO myUtile VALUE (?)", reftime)

		// Retrieve time from DB
		rows := dbt.mustQuery("SELECT ts FROM myUtile")
		if !rows.Next() {
...
func TestSQLInjection(t *testing.T) {
	createTest := func(arg string) func(dbt *DBTest) {
		return func(dbt *DBTest) {
			dbt.mustExec("CREATE TABLE myUtile (v INTEGER)")
			dbt.mustExec("INSERT INTO myUtile VALUES (?)", 1)

			var v int
			// NULL can't be equal to anything, the idea here is to inject query so it returns row
			// This myUtile verifies that escapeQuotes and escapeBackslash are working properly
			err := dbt.db.QueryRow("SELECT v FROM myUtile WHERE NULL = ?", arg).Scan(&v)
...
func TestInsertRetrieveEscapedData(t *testing.T) {
	testData := func(dbt *DBTest) {
		dbt.mustExec("CREATE TABLE myUtile (v VARCHAR(255))")

		// All sequences that are escaped by escapeQuotes and escapeBackslash
		v := "foo \x00\n\r\x1a\"'\\"
		dbt.mustExec("INSERT INTO myUtile VALUES (?)", v)

		var out string
		err := dbt.db.QueryRow("SELECT v FROM myUtile").Scan(&out)
		if err != nil {
			dbt.Fatalf("%s", err.Error())
		}
...
func TestRejectReadOnly(t *testing.T) {
	runTests(t, dsn, func(dbt *DBTest) {
		...
		// Error 1193: Unknown system variable 'TRANSACTION' => skip myUtile,
		// MySQL server version is too old
		maybeSkip(t, err, 1193)
		if _, err := dbt.db.Exec("DROP TABLE myUtile"); err == nil {
	...
	runTests(t, dsn+"&rejectReadOnly=true", func(dbt *DBTest) {
		...
		dbt.mustExec("DROP TABLE myUtile")
	})
}
