...
func newCacheWithJanitor(de time.Duration, ci time.Duration, m map[string]Item) *L1Cache {
	c := newCache(de, m)
...
func NewCache(defaultExpiration, cleanupInterval time.Duration) *L1Cache {
...
func NewCacheFrom(defaultExpiration, cleanupInterval time.Duration, items map[string]Item) *L1Cache {
	return newCacheWithJanitor(defaultExpiration, cleanupInterval, items)
}